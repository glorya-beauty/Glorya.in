...
@section('title', 'Checkout - Glorya Beauty')
...
<!-- Booking Section -->
<section id="booking-section" class="contact-area contact-bg pt-100 pb-100 p-relative fix" style="background-image:url({{ asset('images/bg/contact-bg.png') }})">
...
<input type="hidden" id="cart-items" value="{{ $cart && $cart->items ? $cart->items->toJson() : '[]' }}">
<input type="hidden" id="cart-subtotal" value="{{ $subtotal ?? 0 }}">
<input type="hidden" id="cart-gst" value="{{ $gstAmount ?? 0 }}">
<input type="hidden" id="cart-total" value="{{ $total ?? 0 }}">
<!-- booking-section-end -->
    ...
    if (bookingData.cartItems.length === 0) {
        summaryContent.innerHTML = `
            <div class="alert alert-warning">
                ...
                <a href="{{ route('services') }}" class="btn btn-sm btn-primary ml-2">Browse Services</a>
            </div>
        `;
        return;
    }
